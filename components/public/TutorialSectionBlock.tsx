...
function Callout({ type, children }: { type: CalloutType; children: React.ReactNode }) {
  ...
  return (
    <div className={`mt-4 flex gap-3 rounded-xl border ${cfg.bg} ${cfg.border} px-4 py-3`}>
      ...
      <div>
        <span className={`text-xs font-bold uppercase tracking-wider ${cfg.labelClass}`}>{cfg.label} — </span>
...
function StepBlock({ index, step, isLast }: { index: number; step: StepItem; isLast: boolean }) {
  return (
    <div className="relative pl-12 pb-8">
      ...
      {step.note && <Callout type="info">{step.note}</Callout>}
      {step.warn && <Callout type="warning">{step.warn}</Callout>}
    </div>
  )
}
  ...
  return (
    <section id={id} className="scroll-mt-24 py-14 lg:py-16 border-b border-border last:border-b-0">
      ...
      <div className={`grid items-start gap-10 ${mockup ? 'lg:grid-cols-[1fr_300px]' : ''}`}>
        <div>
          {steps.map((step, i) => (
            <StepBlock key={step.t} index={i + 1} step={step} isLast={i === steps.length - 1} />
          ))}
        </div>
