...
export default function GroupCard({ group, locale }: Props) {
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[24px] border border-line bg-surface transition-colors duration-200 hover:border-text/20">
      <Link href={`/${locale}/group/${group.id}`} className="flex h-full flex-col">
        <div className="border-b border-line px-5 pb-5 pt-6">
          <div className="flex items-start gap-4">
            <div className="relative flex h-[72px] w-[72px] shrink-0 items-center justify-center overflow-hidden rounded-[22px] border border-line bg-surface shadow-sm">
              {group.logoUrl ? (
                <Image
                  src={group.logoUrl}
                  alt={group.name}
            ...
            <div className="min-w-0 flex-1">
              ...
              <h3 className="mt-3 text-[22px] font-semibold tracking-[-0.03em] text-ink">
                {group.name}
              </h3>
