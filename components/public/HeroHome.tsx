...
export default function HeroHome({ copy, stats, locale }: { copy: any; stats: any; locale: string }) {
  return (
        ...
        <div className="grid items-center gap-12 lg:grid-cols-[1fr_1fr] lg:gap-10">
          ...
          <div className="space-y-8 lg:space-y-10">
            ...
            <div className="flex flex-wrap items-center gap-6 pt-2 emil-enter-stagger emil-stagger-3">
              ...
              <Link
                href={`/${locale}/app`}
                className="inline-flex h-14 items-center gap-2 rounded-2xl border border-border bg-white px-6 text-sm font-bold text-ink transition-colors duration-150 ease-[var(--ease-out)] hover:border-accent/40 hover:text-accent"
              >
                {copy.heroCtaApp}
              </Link>
            </div>

            {/* Stats — woven below the CTA row */}
            <div className="flex items-center gap-5 pt-2 emil-enter-stagger emil-stagger-3">
              {[
          ...
          <div className="relative flex justify-center lg:justify-end emil-enter-stagger emil-stagger-4">
            ...
            >
              <FeatureMockup type="map" size="lg" />
            </div>
          </div>
